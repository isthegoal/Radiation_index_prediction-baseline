...
def UseLightGBM():
    ...
    train=all_data[all_data['belong']==0]
    test = all_data[all_data['belong'] == 1]
    y=train.pop('电场实际太阳辐射指数')
    #
    # test = pd.read_csv( './data/extract_test_data.csv')
    # y = train.pop('label')

    features = [i for i in train.columns if i not in ['日期','电场实际太阳辐射指数']]
    ...
    for train_index, test_index in kf.split(train):
        ...
        params = { 'metric': 'mae',  'boosting_type': 'gbdt',  'learning_rate': 0.01,  'n_estimators':100,
